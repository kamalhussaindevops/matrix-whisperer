...
const WhatIsDestinyMatrix = ({ content }: { content?: MarkdownPage }) => {
  const title       = content?.title       ?? "What is the Destiny Matrix?";
  const subtitle    = content?.subtitle    ?? "A modern numerological system that decodes the energies present at your birth.";
  const badge       = content?.badge       ?? "Introduction";
    ...
    <div className="flex min-h-screen flex-col bg-background">
      ...
      <main className="flex-1 py-16">
        <article className="container mx-auto max-w-2xl px-4">
          ...
          <div
            className="prose-custom space-y-6 text-muted-foreground leading-relaxed [&_h2]:font-display [&_h2]:text-2xl [&_h2]:text-foreground [&_h2]:mt-10 [&_h2]:mb-4 [&_p]:text-base [&_strong]:text-foreground"
